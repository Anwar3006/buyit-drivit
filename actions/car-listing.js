...
export async function getAllSavedCarsForUser() {
  try {
    const { userId } = await auth();
    if (!userId) {
      throw new Error("Unauthenticated. Please login");
    ...
    console.error("Error getting saved cars: " + error.message);
    return {
      success: true,
      error: error.message,
    };
        ...
        testDriveInfo: {
          ...
          dealership: dealership
            ? {
                ...
                upatedAt: dealership?.updatedAt?.toISOString(),
                workingHours: dealership?.workingHours?.map((hour) => ({
                  ...hour,
                  createdAt: hour.createdAt.toISOString(),
                  upatedAt: hour.updatedAt.toISOString(),
                })),
              }
            : null,
        },
