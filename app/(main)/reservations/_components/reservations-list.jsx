...
const ReservationsList = ({ initialData }) => {
  ...
  const upcomingBookings = initialData?.data?.filter((bookings) =>
    ["PENDING", "CONFIRMED"].includes(bookings.status)
  );
  const pastBookings = initialData?.data?.filter((bookings) =>
    ["COMPLETED", "CANCELLED", "NO_SHOW"].includes(bookings.status)
  );
